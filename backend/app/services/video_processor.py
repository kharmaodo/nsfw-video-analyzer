    ...
    async def run(self, command: list[str], timeout_seconds: float) -> CommandResult:
        try:
            ...
            raise VideoProcessingError(
                f"Exécutable indisponible : {command[0]}."
            ) from exc
...
def calculate_central_window(
    ...
    return SampleWindow(
        start_seconds=max(0.0, (source_duration_seconds - duration) / 2),
...
class VideoProcessor:
    ...
    async def probe_duration(self, source: str | Path) -> float:
        value, is_local = await self._source_value(source)
        return await self._probe_duration(value, is_local)
    ...
    async def extract_central_frames(
        ...
        value, is_local = await self._source_value(source)
        duration = await self._probe_duration(value, is_local)
        ...
        output_directory = (
            Path(self.settings.video_temporary_directory) / f"video-{video_id}"
        )
        ...
        if not is_local:
            command.extend(["-protocol_whitelist", self.PROTOCOL_WHITELIST])
        command.extend(
            [
                "-ss",
                ...
                "-vf",
                (
                    f"fps=1/{self.settings.video_frame_interval_seconds:g},"
                    f"scale={self.settings.video_frame_width}:-2"
                ),
                "-an",
                "-fps_mode",
                "vfr",
    ...
    async def _source_value(self, source: str | Path) -> tuple[str, bool]:
        if isinstance(source, Path):
            path = source.resolve()
            if not path.is_file():
                raise VideoProcessingError("Le média local est introuvable.")
            return str(path), True

        await self.security.validate(source)
        return source, False

    async def _probe_duration(self, value: str, is_local: bool) -> float:
        command = [
        ...
        result = await self.runner.run(
            command,
            self.settings.video_process_timeout_seconds,
        )
        try:
            duration = float(json.loads(result.stdout)["format"]["duration"])
        except (KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
            raise VideoProcessingError(
                "ffprobe n’a pas retourné une durée valide."
            ) from exc

        if duration <= 0:
            raise VideoProcessingError(
                "ffprobe a retourné une durée non positive."
            )
        return duration
